...
function list_admin_institutes_partner_registered_content()
{
    ...
    if (isset($_GET['action']) && !empty($_GET['action'])) {
        ...
        } else if ($_GET['action'] == 'institute-students-payments') {
            global $wpdb;
...
class TT_Institute_Students_List_Table extends WP_List_Table
{
    ...
    function column_default($item, $column_name)
    {
        switch ($column_name) {
            ...
            case 'payments':
                $buttons = '';
                $buttons .= "<a style='margin: 1px' href='" . admin_url('/admin.php?page=list_admin_institutes_partner_registered_content&action=institute-students-payments&student_id=' . $item['id'] . '&institute_id=' . $_GET['institute_id']) . "' class='button button-primary'>" . __('View payments', 'edusystem') . "</a>";
                return $buttons;
            default:
    ...
    function get_columns()
    {
        ...
        $columns = array(
            ...
            'created_at' => __('Created at', 'edusystem'),
            'payments' => __('Payments', 'edusystem'),
        );
