...
function add_admin_form_school_subjects_content()
{
    ...
    if (isset($_GET['section_tab']) && !empty($_GET['section_tab'])) {
        ...
        if ($_GET['section_tab'] == 'add_subject_equivalence') {
            $teachers = get_teachers_active();
            $equivalence = true;
            include (plugin_dir_path(__FILE__) . 'templates/school-subject-detail.php');
        }

        if ($_GET['section_tab'] == 'equivalence_subjects') {
            $list_school_subjects = new TT_school_equivalence_subjects_all_List_Table;
...
class TT_school_equivalence_subjects_all_List_Table extends WP_List_Table
{
    ...
    function column_default($item, $column_name)
    {
        ...
        switch ($column_name) {
            ...
            case 'view_details':
                return "<a href='" . admin_url('/admin.php?page=add_admin_form_school_subjects_content&section_tab=subject_details&equivalence=true&subject_id=' . $item['school_subject_id']) . "' class='button button-primary'>" . __('View Details', 'aes') . "</a>";
            default:
                return ucwords($item[$column_name]);
        }
    }
    ...
    function get_school_subject_pendings()
    {
        ...
        $school_subjects = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS * FROM wp_equivalence_subjects {$query_search} {$query_electives} ORDER BY id DESC LIMIT {$per_page} OFFSET {$offset}", "ARRAY_A");
