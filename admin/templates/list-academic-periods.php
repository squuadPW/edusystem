	...
	<div class="wrap">
		...
		<div style="display:flex;width:100%;justify-content:end;margin-bottom:10px;">
			<a href="<?= admin_url('admin.php?page=add_admin_form_academic_periods_content&section_tab=add_period'); ?>"
				class="button button-outline-primary"><?= __('Add Period', 'edusystem'); ?></a>
			<a style="margin-left: 5px"
				href="<?= admin_url('admin.php?page=add_admin_form_academic_periods_content&action=generate_next_period'); ?>"
				class="button button-outline-primary"><?= __('Generate next academic period available', 'edusystem'); ?></a>
		</div>
		<form action="" id="post-filter" method="post">
			<p class="search-box">
				...
				<input type="search" id="search-box-id-search-input" name="s"
					placeholder="<?= __('Search for Period', 'edusystem'); ?>"
