<h2 class="nav-tab-wrapper">
	...
	<a href="<?= admin_url('admin.php?page=add_admin_institutes_content&section_tab=all_institutes'); ?>" class="nav-tab <?= (isset($_GET['section_tab']) && !empty($_GET['section_tab']) && $_GET['section_tab'] == 'all_institutes') ? 'nav-tab-active' : ''; ?>"><?= __('All Approved Institutes','form-plugin'); ?></a>
</h2>
<div class="tabs-content">
	<div class="wrap">
        <div style="text-align:start;">
            <?php if(!isset($_GET['section_tab'])): ?>
                <h1 class="wp-heading-line"><?= __('Institutes for Review','aes'); ?></h1>
            <?php elseif(isset($_GET['section_tab']) && !empty($_GET['section_tab']) && $_GET['section_tab'] == 'all_institutes'): ?>
                <h1 class="wp-heading-line"><?= __('All Approved Institutes','aes'); ?></h1>
            <?php endif; ?>
        </div>
		...
		<?php if(isset($_GET['section_tab']) && !empty($_GET['section_tab']) && $_GET['section_tab'] == 'all_institutes'): ?>
			<div style="display:flex;width:100%;justify-content:end;margin-bottom:10px;">
				<a href="<?= admin_url('admin.php?page=add_admin_institutes_content&section_tab=add_institute'); ?>" class="button button-outline-primary"><?= __('Add Institute','aes'); ?></a>
			</div>
		<?php endif; ?>
        <form action="" id="post-filter" method="post">
			<p class="search-box">
				<label class="screen-reader-text" for="search-box-id-search-input"><?= __('Search','aes').':'; ?></label>
				<input type="search" id="search-box-id-search-input" name="s" value="<?= (!empty($_POST['s'])) ? $_POST['s'] : ''; ?>" placeholder="<?= __('Search by name, email, recto\'s first name or rector\'s last name','aes'); ?>">
