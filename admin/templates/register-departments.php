            ...
            <div class="capability-group" >
                ...
                <ul>
                    ...
                    <li class="capability-item">
                            ...
                            <?php $capability_disabled = !in_array('manager_admission_aes', $departments_subscription); ?>
                            <label for="manager_admission_aes" <?= $capability_disabled ? 'capability-disabled' : '' ?>>

                                <input id="manager_admission_aes" <?= $capability_disabled ? 'capability-disabled' : '' ?> type="checkbox" name="capabilities[]"
                                    value="manager_admission_aes"  <?= (isset($capabilities) && !empty($capabilities) && isset($capabilities['manager_admission_aes'])) ? 'checked' : ''; ?>>
                                <?= __('Manage Admission', 'edusystem'); ?>
                            </label>
                        

                        <ul>

                            <?php $manager_documents_aes = !in_array('manager_documents_aes', $departments_subscription); ?>
                            <label for="manager_documents_aes" <?= $manager_documents_aes ? 'capability-disabled' : '' ?>>

                                <input id="manager_documents_aes" <?= $manager_documents_aes ? 'capability-disabled' : '' ?> type="checkbox" name="capabilities[]"
                                    value="manager_documents_aes"  <?= (isset($capabilities) && !empty($capabilities) && isset($capabilities['manager_documents_aes'])) ? 'checked' : ''; ?>>
                                <?= __('Required Documents', 'edusystem'); ?>
                            </label>
...
<?php
    ...
    function render_capability_checkbox($cap_key, $label, $departments_subscription, $capabilities) {
        $disabled = !in_array($cap_key, $departments_subscription);
        $checked = !empty($capabilities[$cap_key] ?? false);
        $disabled_class = $disabled ? 'capability-disabled' : '';
            
        return '
            <label for="' . esc_attr($cap_key) . '" class="' . $disabled_class . '">
                <input id="' . esc_attr($cap_key) . '" class="' . $disabled_class . '" 
                    type="checkbox" name="capabilities[]" value="' . esc_attr($cap_key) . '" 
                    ' . ($checked ? 'checked' : '') . '>
                ' . esc_html__($label, 'edusystem') . '
            </label>';
    }
?>
?>