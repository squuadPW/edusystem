...
esta no me trae las fecha q son
SELECT 
   MIN(apc.start_date) AS start_date,
   MAX(apc.end_date)   AS end_date
FROM wp_academic_periods_cut apc
WHERE (apc.code = '20232024' AND apc.cut = 'A')
   OR (apc.code = '20242025' AND apc.cut = 'A');
   ...
   SELECT 
    ...
    sd.automatic,
    sd.upload_at
FROM wp_student_documents sd
WHERE sd.upload_at BETWEEN
    (
        SELECT MIN(apc.start_date)
        FROM wp_academic_periods_cut apc
        WHERE (apc.code = '20232024' AND apc.cut = 'A')
           OR (apc.code = '20242025' AND apc.cut = 'A')
    )
