document.addEventListener("DOMContentLoaded", function () {
  if (document.getElementById("continue-checkout")) {
    document
      .getElementById("continue-checkout")
      .addEventListener("click", function (event) {
        ...
        XHR.open(
          "POST",
          `${ajax_object.ajax_url}?action=use_previous_form_aes`,
          true
        );
        ...
        XHR.onload = function () {
          if (XHR.status === 200) {
            if (XHR.response) {
              ...
              const emailStudent = getCookie("email_student");
              let emailParent = getCookie("email_partner"); // Asumiendo que 'email_partner' es el email_parent

              // Validación: si email_student es igual a email_parent, pon null al email_parent
              if (emailStudent === emailParent) {
                emailParent = null; // Asigna null a la variable si son iguales
              }
              ...
              ).value = getCookie("middle_last_name_student");
              ...
              ).value = getCookie("parent_document_type");
              ...
              document.querySelector("input[name=email_partner]").value =
                emailParent; // Usa la variable que podría ser null
              document.querySelector("input[name=password]").value =
                getCookie("password");
              document.getElementById("modal-continue-checkout").style.display =
                "none";
            }
          } else {
