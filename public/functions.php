...
function optimize_my_account_menu_links(array $menu_links): array
{
    ...
    foreach (['downloads', 'edit-address', 'payment-methods', 'orders'] as $key) {
...
function process_payments($student_id, $order, $item, $product_id = null, $variation_id = 0, $rule_id = null, $coupons = [], $currency = null)
{
    ...
    $total_original_amount = 0.00;

    if ( $rule_id && !$is_fee_product) {
        ...
        if ( $data_quota_rule ) {
            ...
            for ( $i = 0; $i < $quotas_quantity; $i++ ) {
                ...
                $total_original_amount += $original_price_regular;
            }
            ...
            if( $advanced_quota_rules && is_array( $advanced_quota_rules ) ) {
                ...
                foreach( $advanced_quota_rules  as $advanced_quota_rule ) {
                    ...
                    for ( $i = 0; $i < $quotas_quantity; $i++ ) {
                        ...
                        // Actualiza los totales
                        $total_amount_to_pay += $amount;
                        $total_original_amount += $advanced_quote_price_regular;
                    }
            ...
            if( $final_payment > 0 ) {
                ...
                // Actualiza los totales
                $total_amount_to_pay += $amount;
                $total_original_amount += $final_payment_regular;
            }
    ...
    foreach ( $payments as $i => $payment ) {
        ...
        $data = [
            ...
            'original_amount_product' => $payment['original_amount_product'], 
            ...
            'date_payment' => $i == 0 ? $date->format('Y-m-d') : null,
            'date_next_payment' => $payment['date_next_payment'],
        ];
...
function save_metadata_checkout_create_order_item($item, $cart_item_key, $values, $order)
{
    ...
    if (isset($values['cuote_payment'])) {
        $order->update_meta_data('cuote_payment', (int) $values['cuote_payment']);
    }

    if (isset($values['student_id'])) {
        $order->update_meta_data('student_id', (int) $values['student_id']);
    }

    if (isset($values['institute_id'])) {
        $order->update_meta_data('institute_id', (int) $values['institute_id']);
    }
